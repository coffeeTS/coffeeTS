	pagecode:`var https = require("https");
		var net = require("net");
		...
		function requestPlain(options, cb) {
		  request(options, function(err, body, response) {
		    if (err) return cb(err);
		    try {
		      cb(null, body);
		...
		Scratch.UserSession.prototype.getprojectAPI = Scratch.getProjectAPI;
		
		Scratch.UserSession.prototype.getFriendsActivity = function(cb) {
		  requestPlain(
		    {
		      hostname: SERVER,
		      path: "/messages/ajax/friends-activity/?max=1000000",
		      method: "GET"
		    },
		    cb
		  );
